...
describe ConceptQL::Operators::PersonFilter do
  it 'behaves itself' do
    ConceptQL::Operators::PersonFilter.new.must_behave_like(:evaluator)
  end
  ...
  describe '#query' do
    it 'uses right stream as argument to PERSON_FILTER against left stream' do
      ...
      ConceptQL::Operators::PersonFilter.new(left: double1, right: double2).query(Sequel.mock).sql.must_equal "SELECT * FROM (SELECT * FROM table1) AS t1 WHERE (person_id IN (SELECT person_id FROM table2 GROUP BY person_id))"
    end
  end
end
