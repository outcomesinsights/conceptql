  ...
  class Spec
        ...
        if CDB.data_model.data_model == :gdm_wide
          ...
          if cols.include?(:criterion_table)
            ds = ds.select(*(cols - [:criterion_table]))
                   .select_append(Sequel.case({ 'observations' => 'clinical_codes' }, :criterion_table,
    ...
    def criteria_counts(test_name, statement = nil)
      load_check(test_name, statement) do |stmt|
        ...
        cq.query.from_self.group_and_count(:criterion_domain).order(:criterion_domain).to_hash(:criterion_domain,
                                                                                               :count)
      end
    end
    ...
    def json_fixture(name)
      json_file = Pathname.new('test') / 'fixtures' / 'json' / "#{name}.json"
      JSON.parse(json_file.read)
    end

    def txt_fixture(name)
      txt_file = Pathname.new('test') / 'fixtures' / 'txt' / "#{name}.txt"
      txt_file.read
    end
