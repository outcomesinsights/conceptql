...
describe ConceptQL::Operators::InformationPeriods do
  ...
  it "should produce correct SQL under gdm" do
    ...
    db.query(["information_periods"]).sql.must_equal "SELECT * FROM (SELECT * FROM (SELECT \"patient_id\" AS \"person_id\", \"id\" AS \"criterion_id\", CAST('information_periods' AS text) AS \"criterion_table\", CAST('observation_period' AS text) AS \"criterion_domain\", \"start_date\", \"end_date\", CAST(NULL AS text) AS \"source_value\" FROM \"information_periods\") AS \"t1\") AS \"t1\""
  end
  ...
  it "should produce correct SQL under omopv4_plus" do
    ...
    db.query(["information_periods"]).sql.must_equal "SELECT * FROM (SELECT * FROM (SELECT \"person_id\" AS \"person_id\", \"observation_period_id\" AS \"criterion_id\", CAST('observation_period' AS text) AS \"criterion_table\", CAST('observation_period' AS text) AS \"criterion_domain\", CAST(\"observation_period_start_date\" AS date) AS \"start_date\", CAST(coalesce(\"observation_period_end_date\", \"observation_period_start_date\") AS date) AS \"end_date\", CAST(NULL AS text) AS \"source_value\" FROM \"observation_period\") AS \"t1\") AS \"t1\""
  end
end
