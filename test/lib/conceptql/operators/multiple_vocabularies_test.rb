...
describe ConceptQL::Operators::MultipleVocabularies do
  ...
  it "should produce correct SQL under omopv4_plus" do
    ...
    db.query(["cpt_or_hcpcs", "99214"]).sql.must_equal "SELECT * FROM (SELECT * FROM (SELECT \"person_id\", \"criterion_id\", \"criterion_table\", \"criterion_domain\", \"start_date\", \"end_date\", \"source_value\", \"source_vocabulary_id\" FROM (SELECT * FROM (SELECT \"person_id\" AS \"person_id\", \"procedure_occurrence_id\" AS \"criterion_id\", CAST('procedure_occurrence' AS text) AS \"criterion_table\", CAST('procedure_occurrence' AS text) AS \"criterion_domain\", CAST(\"procedure_date\" AS date) AS \"start_date\", CAST(coalesce(\"procedure_date\", \"procedure_date\") AS date) AS \"end_date\", CAST(\"procedure_source_value\" AS text) AS \"source_value\", CAST(\"procedure_source_vocabulary_id\" AS integer) AS \"source_vocabulary_id\" FROM \"procedure_occurrence\" WHERE ((\"procedure_source_vocabulary_id\" = 4) AND (\"procedure_source_value\" IN ('99214')))) AS \"t1\" UNION (SELECT * FROM (SELECT \"person_id\" AS \"person_id\", \"procedure_occurrence_id\" AS \"criterion_id\", CAST('procedure_occurrence' AS text) AS \"criterion_table\", CAST('procedure_occurrence' AS text) AS \"criterion_domain\", CAST(\"procedure_date\" AS date) AS \"start_date\", CAST(coalesce(\"procedure_date\", \"procedure_date\") AS date) AS \"end_date\", CAST(\"procedure_source_value\" AS text) AS \"source_value\", CAST(\"procedure_source_vocabulary_id\" AS integer) AS \"source_vocabulary_id\" FROM \"procedure_occurrence\" WHERE ((\"procedure_source_vocabulary_id\" = 5) AND (\"procedure_source_value\" IN ('99214')))) AS \"t1\")) AS \"t1\") AS \"t1\") AS \"t1\""
  end
