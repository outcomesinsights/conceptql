...
describe ConceptQL::Operators::Read do
  ...
  it "should produce correct SQL under omopv4_plus" do
    ...
    db.query(["read", "xyz"]).sql.must_equal "SELECT * FROM (SELECT * FROM (SELECT \"person_id\", \"criterion_id\", \"criterion_table\", \"criterion_domain\", \"start_date\", \"end_date\", \"source_value\", \"source_vocabulary_id\" FROM (SELECT * FROM (SELECT \"person_id\" AS \"person_id\", \"observation_id\" AS \"criterion_id\", CAST('observation' AS text) AS \"criterion_table\", CAST('observation' AS text) AS \"criterion_domain\", CAST(\"observation_date\" AS date) AS \"start_date\", CAST(coalesce(\"observation_date\", \"observation_date\") AS date) AS \"end_date\", CAST(\"observation_source_value\" AS text) AS \"source_value\", CAST(\"observation_source_vocabulary_id\" AS integer) AS \"source_vocabulary_id\" FROM \"observation\" AS \"tab\" WHERE ((\"observation_source_value\" IN ('xyz')) AND (\"observation_source_vocabulary_id\" = 17))) AS \"t1\") AS \"t1\") AS \"t1\") AS \"t1\""
  end
end
