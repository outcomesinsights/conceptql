module ConceptQL
  module Provenanceable
    ...
    def prov_of(db, ancestors)
      dm.descendants_of(db, dm.concept_ids(db, [dm.file_provenance_types_vocab, dm.code_provenance_types_vocab].flatten, ancestors))
    end
    ...
    def find_bad_keywords(db, codes) 
      codes - dm.concepts(db, [dm.file_provenance_types_vocab, dm.code_provenance_types_vocab].flatten).select_map(:concept_code)
    end
  end
end
