        ...
        def modified_query
          if dm.table_cols(source_table).include?(:context_id)
            query.from_self(alias: :c)
                 ...
                 .join(Sequel[:practitioners].as(:p), practitioner_id: :practitioner_id)
                 .select_all(:c)
                 .select_append(
                   Sequel[:cp][:practitioner_id].as(:provider_id),
                   Sequel[:p][:role_concept_id].as(:role_concept_id),
                   Sequel[:cp][:specialty_type_concept_id].as(:context_specialty_concept_id),
                   Sequel[:p][:specialty_concept_id].as(:provider_specialty_concept_id)
                 )
          else
            query
              .select_all
